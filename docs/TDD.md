...
| run_container | container run ... | Map memory, cpus, ports, env, volumes, and init_image arguments to flags. Also `--read-only-path` / `--masked-path` (1.2.1+, additive-only — see 1.2.1 audit note below). |
| exec_in_container | container exec ... | Run commands inside running containers. |
| export_container | container export -o [file] [id] | Export container filesystem as an OCI-layout tar archive. Works on running containers as of 1.2.1. |
| build_image | container build ... | Build images asynchronously. Supports `--secret` and `--ssh` (1.2.1+, build-scoped agent forwarding). |
...
| set_machine | container machine set ... | Update machine config (cpus/memory/home-mount, and `virtualization=<bool>` on 1.1+, version-gated); effective after restart. (Apple Container 1.0+) |
...
* **1.2.1 audit — read-only/masked paths, build SSH forwarding, k8s plugin:** 1.2.1 turned the 1.2.0 `maskedPaths`/`readonlyPaths` Swift API groundwork into CLI-facing flags: `--read-only-path` and `--masked-path` on `run`/`create` (EXPERIMENTAL upstream). Both are repeatable and *additive* by default — they extend the runtime's default protected-path lists rather than replace them — so `run_container` exposes them directly as `read_only_paths`/`masked_paths`. The CLI also accepts a `NONE` sentinel there that clears the defaults instead of adding to them, which weakens the guest's isolation the same way `--cap-add`/`--cap-drop` do; `run_container` rejects that literal value (case-insensitive) and only supports the additive form. Separately, 1.2.1 added `--ssh <value>` to `build` (forwards the host SSH agent to a `RUN --mount=type=ssh` step, build-scoped only) — this is exposed as `build_image`'s `ssh` parameter and is unrelated to `run`'s own `--ssh`, which remains blocklisted (it forwards the agent for the container's whole running lifetime, a materially larger exposure window). 1.2.1 also shipped an EXPERIMENTAL `container k8s` plugin (local Kubernetes clusters: create/list/delete/start/load-image/write-config) and 1.2.2 fixed a bug preventing it from loading when installed via the release package; it is not bundled with the Homebrew formula on the machine this was validated on and is not wrapped by this server. Validated against the installed 1.2.2 binary.
