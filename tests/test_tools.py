from apple_container_mcp.tools import check_apiserver_status, run_container, stop_container, start_container, get_logs, active_builds, build_image, check_build_status, create_network, create_volume, prune_images
...
def test_run_container_with_options(mocker):
    ...
    result = run_container("ubuntu", cpus=2, memory="4g", name="my-ubuntu", detach=False, ports=["8080:80"], env=["DEBUG=1"], volumes=["/tmp/host:/tmp/container"], network="my-net")
    assert result == {"status": "ok", "id": "67890"}
    mock_cmd.assert_called_once_with(["run", "--name", "my-ubuntu", "--cpus", "2", "--memory", "4g", "--network", "my-net", "-p", "8080:80", "-e", "DEBUG=1", "-v", "/tmp/host:/tmp/container", "ubuntu"])
...
def test_run_container_error(mocker):
    ...
    assert result == {"status": "error", "message": "Failed to run container", "details": "Image not found", "exit_code": 125}
